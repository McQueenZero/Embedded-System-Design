...
// PB8
	...
	Nixietube_GPIO.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	Nixietube_GPIO.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
	GPIO_Init(GPIOE, &Nixietube_GPIO);				 //根据设定参数初始化
...
void Exti_line_Init(void)
{
	...
	NVIC_function.NVIC_IRQChannel = EXTI15_10_IRQn;			    //中断通道6
